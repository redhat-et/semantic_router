...
func (c *StdioClient) loadTools(ctx context.Context) error {
	...
	if c.promptGuard != nil && c.promptGuard.IsEnabled() {
		c.log(LoggingLevelInfo, "Analyzing tools for potential security threats...")
		
		analysisResults, err := c.promptGuard.AnalyzeTools(filteredTools)
		if err != nil {
			c.log(LoggingLevelWarning, fmt.Sprintf("Error analyzing tools: %v", err))
		} else {
			...
			for _, result := range analysisResults {
				if result.IsJailbreak {
					maliciousCount++
					c.log(LoggingLevelError, fmt.Sprintf("SECURITY THREAT: Tool '%s' flagged as %s (confidence: %.3f)", 
						result.ToolName, result.JailbreakType, result.Confidence))
				}
			}
			
			if maliciousCount > 0 {
				c.log(LoggingLevelError, fmt.Sprintf("SECURITY ALERT: %d potentially malicious tools detected", maliciousCount))
			}
		}
	}
...
func (c *StdioClient) CallTool(ctx context.Context, name string, arguments map[string]interface{}) (*mcp.CallToolResult, error) {
	...
	if c.promptGuard != nil && c.promptGuard.IsEnabled() {
		textContents := ExtractTextFromArguments(arguments)
		if len(textContents) > 0 {
			hasJailbreak, detections, err := c.promptGuard.AnalyzeContent(textContents)
			if err != nil {
				c.log(LoggingLevelWarning, fmt.Sprintf("Error performing jailbreak analysis: %v", err))
			} else if hasJailbreak {
				// Find the first jailbreak detection for logging
				...
				c.log(LoggingLevelError, fmt.Sprintf("JAILBREAK ATTEMPT BLOCKED in tool call '%s': %s (confidence: %.3f)", name, jailbreakType, confidence))
				return nil, fmt.Errorf("tool call blocked due to potential jailbreak attempt (type: %s, confidence: %.3f)", jailbreakType, confidence)
			}
		}
	}
	...
	result, err := c.mcpClient.CallTool(ctx, mcp.CallToolRequest{
		Params: mcp.CallToolParams{
			Name:      name,
			Arguments: arguments, // Pass arguments directly as map[string]interface{}
		},
	})
	...
	if c.promptGuard != nil && c.promptGuard.IsEnabled() {
		...
		if len(textContents) > 0 {
			...
			if err != nil {
				...
				c.log(LoggingLevelError, fmt.Sprintf("JAILBREAK ATTEMPT BLOCKED in prompt '%s': %s (confidence: %.3f)", name, jailbreakType, confidence))
				return nil, fmt.Errorf("prompt access blocked due to potential jailbreak attempt (type: %s, confidence: %.3f)", jailbreakType, confidence)
			}
		}
	}
